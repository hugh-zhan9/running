...
import type { Map as MapInstance, FilterSpecification } from 'maplibre-gl';
...
const RunMap = ({
  ...
  const filterProvinces = useMemo<FilterSpecification>(
    () => ['in', 'name', ...provinces],
    [provinces]
  );
  const filterCountries = useMemo<FilterSpecification>(
    () => ['in', 'name', ...countries],
    [countries]
  );
